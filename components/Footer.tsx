...
const SOCIALS = [
  { icon: Instagram, tint: "hover:bg-cherry hover:text-cream" },
  { icon: MessageCircle, tint: "hover:bg-sky hover:text-ink" },
  { icon: Mail, tint: "hover:bg-gold hover:text-ink" },
  { icon: Phone, tint: "hover:bg-forest hover:text-cream" },
];

const STRIPE_COLORS = ["bg-forest", "bg-cherry", "bg-gold", "bg-sky", "bg-tangerine"];

export default function Footer() {
  return (
    <footer className="bg-ink text-cream mt-24">
      {/* Thin multi-color accent strip */}
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10 py-16 grid grid-cols-1 md:grid-cols-4 gap-12">
        ...
        >
          <h4 className="font-semibold mb-4">Quick Links</h4>
          <ul className="space-y-2 text-sm text-cream/70">
            <li><Link href="/registration" className="hover:text-gold transition-colors">Registration</Link></li>
            <li><Link href="/points-table" className="hover:text-gold transition-colors">Points Table</Link></li>
            <li><Link href="/schedule" className="hover:text-gold transition-colors">Schedule</Link></li>
            <li><Link href="/rules" className="hover:text-gold transition-colors">Rules</Link></li>
          </ul>
        </motion.div>
        ...
        >
          ...
          <form className="flex gap-2">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.94 }}
              className="bg-gold text-ink rounded-full px-4 py-2 text-sm font-semibold"
            >
              Join
            </motion.button>
          </form>
        ...
        >
          <h4 className="font-semibold mb-4">Connect</h4>
          <div className="flex gap-3">
            {SOCIALS.map(({ icon: Icon, tint }, i) => (
              <motion.a
                key={i}
                href="#"
                whileHover={{ scale: 1.12, rotate: 8 }}
                whileTap={{ scale: 0.94 }}
                className={`w-10 h-10 rounded-full bg-cream/10 flex items-center justify-center transition-colors ${tint}`}
              >
                <Icon size={18} />
              </motion.a>
            ))}
