...
export async function fetchLeagueStandings(): Promise<LeaguePlayer[]> {
  const rows = await fetchRows(SHEETS_CONFIG.GIDS.league);
  return rows
    .filter((r) => get(r, "Players"))
    .map((r) => ({
      name: get(r, "Players"),
      matchesPlayed: Number(get(r, "Matches Played")) || 0,
      wins: Number(get(r, "Wins")) || 0,
      points: Number(get(r, "Points")) || 0,
    }));
}
...
export async function fetchChampionships(): Promise<Championship[]> {
  ...
  for (const r of rows) {
    ...
    // Podiums/Highscore are only relevant for some games. We still parse
    // them if present in the row, but leave them undefined if the cell is
    // blank — the UI decides per-game which of these to actually display.
    const podiumsRaw = get(r, "Podiums");
    const highscoreRaw = get(r, "Highscore");
    ...
    map.get(name)!.standings.push({
      ...
      wins: Number(get(r, "Wins")) || 0,
      points: Number(get(r, "Points")) || 0,
      ...(podiumsRaw ? { podiums: Number(podiumsRaw) || 0 } : {}),
      ...(highscoreRaw ? { highscore: Number(highscoreRaw) || 0 } : {}),
    });
  }
